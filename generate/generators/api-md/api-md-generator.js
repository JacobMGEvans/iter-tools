...
const { syncName, renameDollar } = require('../../names');
const extractMethodSignature = require('./extract-method-signature');
...
class MonoliticGenerator extends Generator {
  ...
  getImplPath(dir, type = null) {
    ...
    const implName = syncName(basename(dir));
  ...
  extractMethodSignature(dir, type) {
    ...
    return file && extractMethodSignature(methodName, file);
  }
    ...
    return [...this.files]
      ...
      .map(([path, content]) => {
        ...
        return {
          ...
          signature: this.extractMethodSignature(dir),
          asyncSignature: this.extractMethodSignature(dir, 'async'),
          parallelSignature: this.extractMethodSignature(dir, 'parallel'),
        };
      });
  }
